...
#eachother and the EPSPS-Cassette subtelomere unit with MUSCLE5 before 
...
tiff("Eindica_subtel_tree.tiff", units="mm", width=180, height=180, res=500)


# Radial Tree #
ggtree(tr=df, layout='circular', size=1) +
  geom_tiplab(align=TRUE, linesize=1.25, geom = "label", fontface = "bold", size=6, aes(colour = c("GlyRes", "Cassette",
                                                                             "GlySus", "GlyRes", 
                                                                             "GlyRes", "GlySus", 
                                                                             "GlyRes", "GlyRes", 
                                                                             "GlySus", "GlySus", 
...
# Legend #
scale_color_manual(values=c(Cassette = "mediumseagreen", GlyRes = "goldenrod3", GlySus = "dodgerblue4")) +
        theme(legend.position = c(0.9,0.6),
        text = element_text(size = 20, face = "bold"),
        legend.title = element_blank(),
        legend.key = element_rect(fill = c("mediumseagreen", "dodgerblue4", "goldenrod3"), linewidth = 20))


# Exits .tff generation #
#comment out if not needed
dev.off()
...
### BONUS TREE ###
#Tree with table replacing legend. Worked well for poster.
