...
The script `Eindica_GRvGS_edgeR_v4.R` calculates differential expression using the two-sided quasi-likelihood F-test in [edgeR](https://bioconductor.org/packages/release/bioc/html/edgeR.html), writes the calculations into `resistant.v.susceptible.txt` and plots it into the below volcano plot.  
